            ...
            >
              ...
              <form @submit.prevent="onSubmit">
                <div class="bg-white px-4 pt-5 pb-4">
                  ...
                  <input
                    v-model="productFeature.slider"
                    type="checkbox"
                    id="slider"
                    :checked="product.slider"
                  />
                  <label for="slider">Slider</label> &nbsp;&nbsp;&nbsp;

                  <input
                    v-model="productFeature.trending"
                    type="checkbox"
                    id="trending"
                    :checked="product.trending"
                  />
                  <label for="slider">Trending</label>
                </div>
                <footer
...
function addSliderImage(e) {
  const file = e;
  productFeature.value.slider_image = file;
...
function onSubmit() {
  ...
  if(Object.keys(productFeature.value).length > 0){
    ...
    if(!Object.keys(productFeature.value).includes('slider')){
      productFeature.value.slider = product.value.slider;
    }

    if(!Object.keys(productFeature.value).includes('trending')){
      productFeature.value.trending = product.value.trending;
    }
    ...
    store.dispatch("sliders/createSlider", productFeature.value).then((res)=>{
      if(res.status === 201){
        store.dispatch("products/getProducts");
        store.dispatch("toast/enableSuccessToast", "Feature added to product!");
        closeModal();
      }else if(res.status === 200){
        store.dispatch("products/getProducts");
        store.dispatch("toast/enableSuccessToast", "Feature updated to product!");
        closeModal();
      } 
    }).catch((err)=>{
