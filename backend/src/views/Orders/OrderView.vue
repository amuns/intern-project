...
onMounted(() => {
  store.dispatch('orders/getOrder', route.params.id)
    .then(({data}) => {
      order.value = data
    })
...
function onStatusChange() {
  axiosClient.post(`/orders/change-status/${order.value.id}/${order.value.status}`)
    .then(({data}) => {
      store.dispatch('toast/enableSuccessToast', `Order status was successfully changed into "${order.value.status}"`)
    })
